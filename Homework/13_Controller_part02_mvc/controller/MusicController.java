...
public class MusicController {
	...
	*/
	...
	public ArrayList<Music> searchMusic(String keyword) {
		...
		// 단, 검색된 곡이 하나도 없을 경우 searchList는 비어있을 것!!
	...
	public int deleteMusic(String title) {
		...
		// 반복문이 다 끝난 시점에 있어서 여전히 result값이 0일 경우 => 삭제할 곡을 찾지 못함..
		//							 result값이 0보다 클 경우 => 뭔가 삭제가 되었단 소리
	...
	public int updateMusic(String title, String upTitle, String upArtist) {
		...
		for(int i=0; i<list.size(); i++) {
			if(list.get(i).getTitle().equals(title)) {
				...
				// list.get(i) == 수정할 곡
				// 방법 1. setter메소드로 변경하기
				/*
				list.get(i).setTitle(upTitle);
				list.get(i).setArtist(upArtist);
				*/
				
				// 방법 2. list에서 제공하는 set메소드 이용
	...
	public ArrayList<Music> sortMusic(int menu) {
		...
		// 복사본 리스트
		ArrayList<Music> sortList = new ArrayList<>();	// 현재 빈 리스트
		sortList.addAll(list);	// list에 기존에 담겨있던 Music객체들 통째로 추가하기
